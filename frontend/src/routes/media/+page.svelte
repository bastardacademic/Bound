  ...
  {#each posts as post}
    <li class:blurred={isCW(post) && !shown[post.id]}>
      {#if isCW(post) && !shown[post.id]}
        <button on:click={() => toggleCW(post.id)} class="reveal">⚠️ Content Warning – Click to Reveal</button>
      {/if}
      <strong>{post.title}</strong><br />
      <CommentThread
        comments={[
          { author: "SceneCurator", text: "This is beautiful work.", timestamp: "1h ago" }
        ]}
